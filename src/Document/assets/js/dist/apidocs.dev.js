...
YUI().use('yuidoc-meta', 'api-list', 'history-hash', 'node-screen', 'node-style', 'pjax', function (Y) {
  var win = Y.config.win,
      ...
      defaultRoute,
      classTabView,
      selectedTab; // Kill pjax functionality unless serving over HTTP.
  ...
  pjax.checkVisibility = function (tab) {
    ...
    var panelNode = tab.get('panelNode'),
        visibleItems; // If no items are visible in the tab panel due to the current visibility
    ...
    if (!visibleItems) {
      if (Y.one('#index .index-item')) {
        panelNode.append('<div class="no-visible-items">' + '<p>' + 'Some items are not shown due to the current visibility ' + 'settings. Use the checkboxes at the upper right of this ' + 'page to change the visibility settings.' + '</p>' + '</div>');
      } else {
        panelNode.append('<div class="no-visible-items">' + '<p>' + 'This class doesn\'t provide any methods, properties, ' + 'attributes, or events.' + '</p>' + '</div>');
      }
  ...
  pjax.initLineNumbers = function () {
    var hash = win.location.hash.substring(1),
        ...
        hasLines,
        node; // Add ids for each line number in the file source view.
  ...
  pjax.updateTabState = function (src) {
    ...
    if (hash && (node = Y.one('#classdocs').getById(hash))) {
      ...
      tab = Y.one('#classdocs .api-class-tab.' + defaultTab); // When the `defaultTab` node isn't found, `localStorage` is stale.
  ...
  pjax.handleClasses = function (req, res, next) {
    var status = res.ioResponse.status; // Handles success and local filesystem XHRs.
  ...
  pjax.onTabSelectionChange = function (e) {
    ...
    selectedTab = tab; // If switching from a previous tab (i.e., this is not the default tab),
