...
YUI.add('api-list', function (Y) {
  var Lang = Y.Lang,
    ...
    keys: {
      ...
      LIST_ITEM_TEMPLATE = '<li class="api-list-item {typeSingular}">' + '<a href="{rootPath}{typePlural}/{name}.html">{displayName}</a>' + '</li>'; // -- Init ---------------------------------------------------------------------
  ...
  }, focusManager, '_focusNext', focusManager); // Create a mapping of tabs in the tabview so we can refer to them easily later.
  ...
  tabviewNode.on('key', onTabSwitchKey, 'down:37,39'); // Focus the filter input when the `/` key is pressed.

  Y.one(Y.config.doc).on('key', onSearchKey, 'down:83'); // Keep the Focus Manager up to date.
  ...
  function getFilterResultNode() {
    ...
    return queryType === 'classes' ? classesNode : queryType === 'elements' ? elementsNode : modulesNode;
  } // -- Event Handlers -----------------------------------------------------------
  ...
  function onFilterResults(e) {
    ...
    if (e.results.length) {
      ...
      frag.append('<li class="message">' + 'No ' + typePlural + ' found.' + '</li>');
    }
  ...
  function onSearchKey(e) {
    ...
    if (target.test('input,select,textarea') || target.get('isContentEditable')) {
      return;
    }
  ...
  function onSearchResults(e) {
    ...
    if (e.results.length) {
      ...
      frag.append('<li class="message">' + 'No results found. Maybe you\'ll have better luck with a ' + 'different query?' + '</li>');
    }
  ...
  function onTabSelectionChange(e) {
    ...
    switch (name) {
      ...
      case 'modules':
        ...
        search.set('minQueryLength', -1); // Only send a request if this isn't the initially-selected tab.
  ...
  requires: ['api-filter', 'api-search', 'event-key', 'node-focusmanager', 'tabview']
});